...
def run_diagnostic(instructions, pointer, input):
    ...
    if output is not None or pointer < 0:
        # print(">",pointer)
...
def get_signal(settings):
    ...
    for i in range(100):
        if i <5:
            ...
            input = [p]
        p, position = run_diagnostic(ins[i%5], positions[i%5], input)
        # if position < 0:
        #     print("****")
